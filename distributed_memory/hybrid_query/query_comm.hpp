...
int prefixExcQuery (std::vector<int> &Out, std::vector<int> &In, unsigned int start, unsigned int end)
{
	Out[start] = 0;
...
std::vector<int> gatherAllLabelsQuery (const std::vector<int> &label_list, std::vector<int> &recv_buffer, int world_size, int world_rank)
{
    ...
    int label_global_sum = prefixExcQuery (world_node_offsets, world_node_list_sizes, 0, world_size);
